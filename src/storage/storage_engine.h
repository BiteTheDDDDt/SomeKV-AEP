...
class StorageEngine {
public:
    StorageEngine(std::string host, std::vector<std::string> peer_host, const std::string& aep_dir,
        ...
        for (auto i : peer_host) {
            ...
            LOG(INFO) << "peerip  " << i.substr(0, x) << "peer port  "
                      << i.substr(x + 1, i.length() - x) << "\n";
            this->peer_host.emplace_back(i.substr(0, x), i.substr(x + 1, i.length() - x));
        }
        int x = host.find_first_of(":");
        std::string port = host.substr(x + 1, host.length() - x - 1);
        LOG(INFO) << "port: " << port << "\n";
        this->port = port;
        netio = std::make_shared<NetworkIO>(
                stoi(port), [&](char* data, int x) { //这里得加上个长度！！！
                    ...
                    int cnt = read_local(select_column, where_column, column_key.data(),
                                         column_key_len, r); // todo 这里要加入把x解析成许多参数
                    //返回的是个数 * size
    ...
    size_t read(int32_t select_column, int32_t where_column, const void* column_key,
        ...
        for (size_t i = 0; i < peer_host.size(); i++) {
            //res +=
            // continue;
            result += netio->sent(this->peer_host[i].first, this->peer_host[i].second, a,
                                  30 + column_key_len); // TODO 这里要把上述参数转化为字符串
        }
